...
The `OpenIDProviderApp.MinimalApi` is the Minimal API counterpart of the `OpenIDProviderApp`: the same Abblix OIDC Server protocol core hosted through the `Abblix.Oidc.Server.MinimalApi` adapter instead of MVC. It configures a signing key and a `client_credentials` client in code and maps the OIDC endpoints with `MapOidcEndpoints()`, making it a compact reference for hosting the server without MVC. Until the Minimal API adapter ships on NuGet, the project builds against the Oidc.Server sources from a sibling checkout.
