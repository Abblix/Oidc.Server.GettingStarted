...
- `dotnet run --project OpenIDProviderApp` starts the provider on `https://localhost:5001`. `AspNetIdentitySample` stands in for it on the same port.
- `dotnet run --project BlazorSample` starts the Blazor app on `https://localhost:5005`.

Open `https://localhost:5005` and sign in (with OpenIDProviderApp the demo user is `john.doe@example.com` / `Jd!2024$3cur3`; with AspNetIdentitySample, register an account from the link on its sign-in screen first). No Node.js is needed: this sample has no client-side bundling step.
