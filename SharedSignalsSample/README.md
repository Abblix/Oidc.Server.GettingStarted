...
The document it does map advertises those management endpoints regardless, because in 2.4 it is built from the route prefix rather than from what was mapped. So a receiver that reads it finds addresses for creating a stream, reading its status, adding and removing subjects and requesting verification, and every one of them answers 404 here. Nothing in this sample notices, because its one receiver is configured rather than discovering - but the configuration document is the single artefact of this pair that a stranger parses by machine, and what it says about this transmitter is not true.
