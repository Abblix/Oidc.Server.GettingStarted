...
`ReceiverApp` sets `RolloverRefetchCooldown` to zero, and the comment beside it says not to copy the line. The default is 30 seconds, and it is a rate limit rather than a propagation delay. The push endpoint carries no transport authentication here, and the issuer allowlist admits any token naming the transmitter, which is a public value. So anyone able to reach the endpoint can send a stream of such tokens naming key ids nobody ever published, and without the floor each one becomes a fetch against the transmitter. The receiver would then work as an amplifier aimed at the party it trusts most.
...
The document it does map advertises those management endpoints regardless, because in 2.4 it is built from the route prefix rather than from what was mapped. So a receiver that reads it finds addresses for creating a stream, reading its status, adding and removing subjects and requesting verification, and every one of them answers 404 here. It also publishes `authorization_schemes` naming OAuth, which announces a guarded management surface on a host that authenticates nobody. Nothing in this sample notices, for the same reason the `2012` warning above costs nothing here - but the configuration document is the single artefact of this pair that a stranger parses by machine, and what it says about this transmitter is not true.
...
Neither endpoint that matters is guarded. Anyone who can reach the transmitter's port can have it sign an event about any session and any user, and anyone who can reach the receiver's push endpoint can submit a token to it. The receiver still refuses what does not verify, so the signature is doing real work here. In a deployment neither endpoint looks like this: there is no HTTP route for revoking, because the dispatch is called in-process from wherever the decision to end a session is taken and already authorised; and the push endpoint carries the credential the stream was registered with, `RequireAuthorization()` on the receiver against `PushAuthorizationHeader` on the transmitter's stream. `RequireAuthorization()` needs both a registered authentication scheme and `AddAuthorization()` behind it, or the host starts cleanly and fails every delivery, which the comment beside `MapPushDeliveryEndpoint` in `ReceiverApp/Program.cs` spells out.
